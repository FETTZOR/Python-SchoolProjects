...
def test_side_calculation_hypotenuse():
    ...
    # c = a2 + b2
    expected = 3.61
    assert round(side_calculation(side1, side2, task), 2) == expected


def test_side_calculation_hypotenuse_exception():
    try:
        side_calculation('x', 4, "hypotenuse")
        assert False
    except TypeError:
        assert True
...
def test_triangle_side_asker():
    with mock.patch('builtins.input', return_value=2):
        assert triangle_side_asker('', 'leg') == 2


def test_triangle_side_asker_hypo():
    with mock.patch('builtins.input', return_value=2):
        assert triangle_side_asker('', 'hypotenuse') == 2
...
def test_ask_from_user_leg():
    ...
    with mock.patch('builtins.input',
                    return_value=actual):
        assert ask_from_user(question, valid_answer) == expected
